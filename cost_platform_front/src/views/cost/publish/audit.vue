        ...
        <el-tabs>
          ...
          <el-tab-pane label="快照对象">
            ...
            <el-collapse class="publish-audit__collapse">
              <el-collapse-item title="费用快照" name="fee"><pre>{{ formatJson(detailData.snapshotGroups?.fees || []) }}</pre></el-collapse-item>
              <el-collapse-item title="变量快照" name="variable"><pre>{{ formatJson(detailData.snapshotGroups?.variables || []) }}</pre></el-collapse-item>
              <el-collapse-item title="规则快照" name="rule"><pre>{{ formatJson(detailData.snapshotGroups?.rules || []) }}</pre></el-collapse-item>
            </el-collapse>
          </el-tab-pane>
        </el-tabs>
      </div>
        ...
        <el-tabs>
          ...
          <el-tab-pane label="费用级差异">
            ...
            <div v-if="selectedFeeDiff" class="publish-audit__diff-detail">
              <div class="publish-audit__bc-section">
                <div class="publish-audit__bc-title">费用主数据</div>
                <div class="publish-audit__bc-header">
                  <span>{{ diffData.fromVersion?.versionNo || '-' }}</span>
                  <span>{{ diffData.toVersion?.versionNo || '-' }}</span>
                </div>
                <div class="publish-audit__bc-body">
                  <div v-for="(row, index) in buildCompareRows(selectedFeeDiff.fromFee, selectedFeeDiff.toFee)" :key="`fee-main-${index}`" class="publish-audit__bc-row">
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.left }}</pre></div>
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.right }}</pre></div>
                  </div>
                </div>
              </div>

              <div class="publish-audit__bc-section">
                <div class="publish-audit__bc-title">关联规则</div>
                <div class="publish-audit__bc-header">
                  <span>{{ diffData.fromVersion?.versionNo || '-' }}</span>
                  <span>{{ diffData.toVersion?.versionNo || '-' }}</span>
                </div>
                <div class="publish-audit__bc-body">
                  <div v-for="(row, index) in buildCompareRows(selectedFeeDiff.fromRules, selectedFeeDiff.toRules)" :key="`fee-rule-${index}`" class="publish-audit__bc-row">
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.left }}</pre></div>
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.right }}</pre></div>
                  </div>
                </div>
              ...
              <div class="publish-audit__bc-section">
                <div class="publish-audit__bc-title">引用变量</div>
                <div class="publish-audit__bc-header">
                  <span>{{ diffData.fromVersion?.versionNo || '-' }}</span>
                  <span>{{ diffData.toVersion?.versionNo || '-' }}</span>
                </div>
                <div class="publish-audit__bc-body">
                  <div v-for="(row, index) in buildCompareRows(selectedFeeDiff.fromVariables, selectedFeeDiff.toVariables)" :key="`fee-variable-${index}`" class="publish-audit__bc-row">
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.left }}</pre></div>
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.right }}</pre></div>
                  </div>
                </div>
          ...
          <el-tab-pane label="规则级差异">
            ...
            <div v-if="selectedRuleDiff" class="publish-audit__diff-detail">
              <div class="publish-audit__bc-section">
                <div class="publish-audit__bc-title">规则主数据</div>
                <div class="publish-audit__bc-header"><span>{{ diffData.fromVersion?.versionNo || '-' }}</span><span>{{ diffData.toVersion?.versionNo || '-' }}</span></div>
                <div class="publish-audit__bc-body">
                  <div v-for="(row, index) in buildCompareRows(selectedRuleDiff.fromRule, selectedRuleDiff.toRule)" :key="`rule-main-${index}`" class="publish-audit__bc-row">
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.left }}</pre></div>
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.right }}</pre></div>
                  </div>
                </div>
              ...
              <div class="publish-audit__bc-section">
                <div class="publish-audit__bc-title">条件明细</div>
                <div class="publish-audit__bc-header"><span>{{ diffData.fromVersion?.versionNo || '-' }}</span><span>{{ diffData.toVersion?.versionNo || '-' }}</span></div>
                <div class="publish-audit__bc-body">
                  <div v-for="(row, index) in buildCompareRows(selectedRuleDiff.fromConditions, selectedRuleDiff.toConditions)" :key="`rule-condition-${index}`" class="publish-audit__bc-row">
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.left }}</pre></div>
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.right }}</pre></div>
                  </div>
                </div>
              ...
              <div class="publish-audit__bc-section">
                <div class="publish-audit__bc-title">阶梯明细</div>
                <div class="publish-audit__bc-header"><span>{{ diffData.fromVersion?.versionNo || '-' }}</span><span>{{ diffData.toVersion?.versionNo || '-' }}</span></div>
                <div class="publish-audit__bc-body">
                  <div v-for="(row, index) in buildCompareRows(selectedRuleDiff.fromTiers, selectedRuleDiff.toTiers)" :key="`rule-tier-${index}`" class="publish-audit__bc-row">
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.left }}</pre></div>
                    <div class="publish-audit__bc-cell" :class="row.same ? 'publish-audit__bc-cell--same' : 'publish-audit__bc-cell--diff'"><pre class="publish-audit__bc-line">{{ row.right }}</pre></div>
                  </div>
                </div>
